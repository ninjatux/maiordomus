            ...
            Promise.each(sshConfig.host, function(address) {
                return new Promise(function (resolve, reject) {
                    ...
                    conn.on('ready', function () {
                        sshLogger('info', stepName, address, maiordomus.$env, 'Connection succesful');
                        maiordomus.$sshSessions.push(conn);
                        resolve();
                    }).on('error', function (err) {
                        sshLogger('error', stepName, address, maiordomus.$env, 'Connection error', err);
                        reject(err);
                    }).on('close', function () {
                        sshLogger('log', stepName, address, maiordomus.$env, 'Closing connection');
                        conn.end();
                    });
    ...
    disconnect: function(args, maiordomus) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (_hasSessions(maiordomus)) {
                ...
                logger.warn(stepName, 'Disconnect called when no ssh session were active');
            }
            ...
            Promise.each(maiordomus.$sshSessions, function (conn) {
                return new Promise(function (resolve, reject) {
                    ...
                    sftp().then(function (sftpClient) {
                        ...
                        .catch(function (err) {
                            sshLogger('error', stepName, conn._host, maiordomus.$env, 'Error getting remote file', err);
                            sftpClient.end();
                            reject(err);
                        });
                    })
