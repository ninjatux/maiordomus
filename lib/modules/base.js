...
function _transform(content, environment, config) {
...
module.exports = {
    ...
    connect: function(args, maiordomus) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (args[0]) {
                logger.user(stepName, _transform(
                    args[0], maiordomus.$env, maiordomus.$config));
            }
    ...
    exec: function(args, maiordomus) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (args[1]) {
                logger.user(stepName, _transform(
                    args[1], maiordomus.$env, maiordomus.$config));
            }

            if (_hasSessions(maiordomus)) {
                Promise.each(maiordomus.$sshSessions, function(conn) {
                    return new Promise(function (resolve, reject) {
                        var cmd = _transform(args[0], maiordomus.$env, maiordomus.$config);
    ...
    get: function (args, maiordomus) {
        return new Promise(function (resolve, reject) {
            ...
            if (args[2]) {
                logger.user(stepName, _transform(args[2], maiordomus.$env, maiordomus.$config));
            }
            ...
            Promise.each(maiordomus.$sshSessions, function (conn) {
                return new Promise(function (resolve, reject) {
                    ...
                    sftp().then(function (sftpClient) {
                        ...
                        var bar;
                        var remotePath = _transform(args[0], maiordomus.$env, maiordomus.$config);
                        var localPath = _transform(args[1], maiordomus.$env, maiordomus.$config);
            ...
            Promise.each(maiordomus.$sshSessions, function (conn) {
                return new Promise(function (resolve, reject) {
                    ...
                    sftp().then(function (sftpClient) {
                        ...
                        var bar;
                        var localPath = _transform(args[0], maiordomus.$env, maiordomus.$config);
                        var remotePath = _transform(args[1], maiordomus.$env, maiordomus.$config);
